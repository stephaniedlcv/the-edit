...
import { getPlannerWeek, type OutfitLogDay, type OutfitLogStatus, type PlannerStats } from "@/lib/planner/data";
import { outfitLogStats as mockStats, outfitLogWeek as mockWeek } from "@/lib/mock-outfit-log";
...
export function PlannerPage() {
  const [entries, setEntries] = useState<OutfitLogDay[]>(mockWeek);
  const [stats, setStats] = useState<PlannerStats>(mockStats);

  useEffect(() => {
    getPlannerWeek()
      .then(({ entries: loaded, stats: loadedStats }) => {
        setEntries(loaded);
  ...
  const modeStats = [
    ...
    {
      value: String(stats.planned).padStart(2, "0"),
      label: "Planned",
          ...
          {entries.map((entry) => (
            ...
            >
              <div>
                ...
                <span
                  className={`mt-5 inline-flex rounded-full border px-3 py-1.5 text-[0.55rem] font-semibold uppercase tracking-[0.18em] ${getStatusClasses(entry.status)}`}
                >
