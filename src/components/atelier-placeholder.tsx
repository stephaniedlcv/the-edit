...
export function AtelierPlaceholder({
  ...
  return (
    <div
      className={`relative isolate overflow-hidden rounded-[3px] border shadow-[inset_0_0_0_1px_rgba(74,47,34,0.08)] ${
        glass
          ? "border-white/50 backdrop-blur-md"
          : "border-white/35"
      } ${className}`}
      style={
        glass
          ? {
              background: `radial-gradient(circle at 22% 18%, rgba(255,255,255,0.22), transparent 30%), linear-gradient(135deg, ${tone.soft}a8, ${tone.base}b0)`,
            }
          : ({
    ...
    >
      ...
      {/* Grid texture */}
      <div
        className="absolute inset-0 [background-image:linear-gradient(90deg,rgba(255,255,255,0.55)_1px,transparent_1px),linear-gradient(0deg,rgba(255,255,255,0.35)_1px,transparent_1px)] [background-size:26px_26px]"
        style={{ opacity: glass ? 0.1 : 0.18 }}
      />
